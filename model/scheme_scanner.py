...
def prepareModel():
    """
    Loads the YOLO model object from files.
    """
    global model
...
def scan(path: str):
    ...
    Args:
        path: File path to the scheme.

    Returns:
        List of dictionaries containing detected token data with coords,
        classes and texts.
    """
    global model
...
def getTokens(image, original, detections, ocr_reader):
    """
    Structurizes tokens received from 'scan'

    Args:
        image: Processed image with detections.
        original: Original image.
        detections: Supervision Detections object with bounding boxes and classes.
        ocr_reader: EasyOCR object for text detection.
